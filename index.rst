...
The screen is illuminated by a projector located at the center of the screen assembly. 
Light from the projector is first reflected by the Calibration Reflector (`tstn-049 <https://tstn-049.lsst.io>`__) and then directed onto the calibration screen. 
...
When the dome is parked, the TMA can point at it with an AZ of -126.2 degrees and Elevation of 23 degrees.
...
In the service position, sufficient clearance exists between the screen and the dome wall to allow a platform lift to access the projector optics and illumination sources located at the center of the screen.
...
System requirements
===================
The calibration screen was designed to achieve optical and mechanical requirements necessary to produce high-quality flat-field calibration images.

* **Flatness Requirement**:
  The RMS flatness of the calibration screen, measured across the entire reflective
  optical surface area, shall not exceed 3 mm when the screen is in the operational
  inclined position.
...
* **Telescope Alignment**:
  The calibration screen system shall include alignment reference features that
  allow the telescope laser tracker system to determine the position of the screen
...
The structure rotates about two pivot points and is driven by a motorized actuator that moves the screen between the service and operational positions.
...
Actuation System
----------------
The calibration screen is designed to rotate between service (vertical) and operational (inclined 23 degrees from vertical) configurations using a mechanical actuator system.

The actuator responsible for this motion has not yet been installed.
